...
exports.distanceRatio = exports.damerauLevenshteinDistance = exports.printMatrix = void 0;
// function to print pretty matrix
function printMatrix(source, target, matrix) {
    source = "##" + source;
    target = "###" + target;
...
function damerauLevenshteinDistance(source, target) {
    ...
    let lastMatchedRowOfTheCharacter = {};
    ...
    for (let row = 1; row <= sourceLength; row++) {
        ...
        for (let col = 1; col <= targetLength; col++) {
            ...
            let lastMatchedRow = lastMatchedRowOfTheCharacter[targetChar];
            ...
            let distaneAdding = distanceMatrix[row][col + 1] + 1;
            let distanceRemoving = distanceMatrix[row + 1][col] + 1;
            let distanceSubstitution = distanceMatrix[row][col] + cost;
            // Calculate the distance for transposition
            let distanceTransposition = distanceMatrix[lastMatchedRow][lastMatchedColumnIndex] +
...
function distanceRatio(source, target) {
    let distance = damerauLevenshteinDistance(source, target);
    let ratio = 1 - (distance / Math.max(source.length, target.length));
    return ratio;
}
exports.distanceRatio = distanceRatio;
